...
from util import frobenius_cost
from util import CreateEnsemblePass
from util import load_ensemble_strs
from bqskit.ir.lang.qasm2 import OPENQASM2Language
from util import  gg_gate_def
...
class AnalyzeEnsemblePass(BasePass):

    def __init__(self) -> None:
        self.lang = OPENQASM2Language(gate_defs=[("gg", gg_gate_def)])
    ...
    async def run(self, circuit: Circuit, data: PassData) -> None:
        ...
        ens_file = ensemble_file_name.format(ind=NUM_ENSEMBLES, extra="")
        ...
        for ens_ind in range(start_ens_ind, NUM_ENSEMBLES):
            ens_file = ensemble_file_name.format(ind=ens_ind, extra="")
            print("Loading Ensemble", ens_file, flush=True)
            circuit_strs = load_ensemble_strs(ens_file)
            print("Number of Circuits", len(circuit_strs), flush=True)

            # Calculate average ensemble error
            circs = [self.lang.decode(c_str) for c_str in circuit_strs]
            counts = [c.count(CNOTGate()) for c in circs]
            uns = [c.get_unitary() for c in circs]
            avg_un = np.mean(uns, axis=0)
            bias = frobenius_cost(avg_un, data.target)
            # Calculate the average error
            dists = [frobenius_cost(u, data.target) for u in uns]
            # Calculate the furthest distance between two us
            ens_dists = [[frobenius_cost(u1, u2) for u2 in uns] for u1 in uns]
            # Get the max distance for each u
            max_dists = [max(dist) for dist in ens_dists]
            # Get the average distance for each u
            avg_dists = [sum(dist) / len(dist) for dist in ens_dists]



            avg_dist = sum(dists) / len(dists)

            # Calculate the variance of the dists
            # var = sum([(d - avg_dist) ** 2 for d in dists]) / len(dists)
            print("Average CNOT Count: ", sum(counts) / len(counts), flush=True)
            print("Average Error: ", avg_dist, flush=True)
            print("Bias: ", bias, flush=True)
            print("Max Error: ", max(dists), flush=True)
            print("Max Distance: ", max(max_dists), flush=True)
            print("Average Distance: ", sum(avg_dists) / len(avg_dists), flush=True)

            stats = [np.mean(counts), avg_dist, bias, max(dists), max(max_dists), np.mean(avg_dists)]
            stats_file = os.path.join(checkpoint_dir, "stats.pkl")
            pickle.dump(stats, open(stats_file, "wb"))
            # print("Variance: ", var, flush=True)
...
base_checkpoint_dir_form = "/pscratch/sd/j/jkalloor/bqskit/block_ens_analysis{extra}"
