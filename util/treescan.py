...
class EnsScanningGateRemovalPass(ScanningGateRemovalPass):
    """
    ...
    async def run(self, circuit: Circuit, data: PassData) -> None:
        ...
        while ops_left:
            ...
            all_circs = EnsScanningGateRemovalPass.get_tree_circs(
            ...
            for i, dist in enumerate(dists):
                if dist < success_threshold:
                    ...
                    all_psols.append((circuit_copy.copy(), dist))
                    if len(all_psols) >= self.max_psols * 5:
                        _logger.debug(
                            f'Found {self.max_psols} solutions, stopping.',
                        ) 
                        # Pick the 10 solutions with the highest distance
                        all_psols = sorted(
                            all_psols,
                            key=lambda x: x[1],
                            reverse=True,
                        )[:self.max_psols]
                        all_psols.append((circuit.copy(), 0.0))
                        data['scan_sols'] = all_psols
                        return
                    break
