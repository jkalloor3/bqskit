...
class GenerateProbabilityPass(BasePass):
    ...
    async def run(
        ...
        for ind in range(3):
            ...
            probs_file = probs_file_name.format(ind=ind, extra=self.checkpoint_extra_str)
            cache_file = cache_file_name.format(ind=ind, extra=self.checkpoint_extra_str)

            if not os.path.exists(ens_file):
                print(f"Ensemble file {ens_file} does not exist, skipping", flush=True)
                continue
            ...
            final_probs_file_1 = f"{checkpoint_dir}/ensemble_{ind}_probs_1_{self.checkpoint_extra_str}.npy"
            # Outer Probability Vector is according to QP on M unique circuits
            final_probs_file_2 = f"{checkpoint_dir}/ensemble_{ind}_probs_2_{self.checkpoint_extra_str}.npy"
            ...
            # final_probs_file_3 = f"{checkpoint_dir}/ensemble_{ind}_probs_3_{self.checkpoint_extra_str}.npy"
            ...
            ensemble_dists: list[list[tuple[np.ndarray, float]]] = await get_runtime().map(
                create_jiggled_unitaries, circ_params, 
                target=target, add_cost=True, drop_zeros=False)
            ...
            print("Max Dist: ", np.max(dists), "Min Dist: ", np.min(dists), flush=True)
            ...
            #     print("Max dist for circuit ", i, ": ", np.max(dists[i]), flush=True)
            #     print("Min dist for circuit ", i, ": ", np.min(dists[i]), flush=True)
            #     print("Threshold: ", self.max_eps, flush=True)
            ...
            #         if np.sum(probs) == 0:
            #             print(f"All probabilities for circuit {i} are 0 - BAD ENSEMBLE", flush=True)
            #             bad_ensembles.append(i)
            #             continue
            ...
            outer_probs_2 = GenerateProbabilityPass.calculate_probs(orig_ensemble, target)
            
            # Calculate joint distribution for seeding FW
            final_probs_1 = np.array([[p * qp_p for p in probs] for probs, qp_p in zip(all_probs, outer_probs_1)])
