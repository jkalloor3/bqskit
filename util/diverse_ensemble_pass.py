...
class DefaultGGEnsemblePass(BasePass):
    ...
    async def run_circuit(self, 
        ...
        circuit_strs = await get_runtime().map(JiggleEnsemblePass.get_final_circ,
                                            circuit_strs,
                                            do_flood_circ=(not self.count_t),
        ...
        if self.count_t:
            ...
            def_params = await get_runtime().map(
                JiggleEnsemblePass.single_jiggle_ham, 
                circuit_strs,
                target=target,
                num=self.num_circs,
                success_threshold=success_threshold
            )
            def_probs = [np.ones((p.shape[0], )) / p.shape[0] for p in def_params]
            all_caches = [None]
    ...
    async def run(self, circuit: Circuit, data: PassData) -> None:
        ...
        if not os.path.exists(ens_file_0):
            print("Missing Checkpoint Directory", flush=True)
            Path(ens_file_0).parent.mkdir(parents=True, exist_ok=True)

        if not os.path.exists(cache_file):
            # Create GG ensemble
            circ = data["e2_scan_sols"][0][0].copy()
            file_names = (ens_file, jiggle_file, probs_file, cache_file)
            await self.run_circuit(circ, file_names, data.target)
        else:
            print("Ensemble already exists, skipping ensemble generation.", flush=True)