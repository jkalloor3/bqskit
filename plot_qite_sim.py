...
if __name__ == '__main__':
    ...
    for i in range(7):
        circ_name = circ_name_text.format(timestep=i)
        ...
        circ_val = get_obs(get_density_matrix(full_sv.numpy))
        print(f"Circuit Value at timestep {i}: {circ_val}")
        circ_vals.append(circ_val)
    ...
    for timestep in range(7):
        ...
        pickle_files = glob.glob(os.path.join(full_path, f"{max_tol}_512.pkl"))
        if len(pickle_files) == 0:
            pickle_files = glob.glob(os.path.join(full_path, f"{max_tol}_128.pkl"))
    ...
    fig.savefig(f'QITE_mag{noise_text}_all_timesteps_dm.png', dpi=300)