...
#SBATCH --output=./logs/{cir_name}_{partitions_size}p_{instantiator}_detached_runtime_{nodes}_nodes_{num_multistarts}_starts-%j.txt
...
#SBATCH --ntasks-per-node=1
#SBATCH --gpus-per-task={amount_of_gpus_per_node}
#SBATCH --output=./logs/{cir_name}_{partitions_size}p_{instantiator}_attached_runtime_{nodes}_nodes_{num_multistarts}_starts-%j.txt
...
def create_and_run_a_job(cir_path, partitions_size, python_file,
    ...
    to_write.write(header_to_use.format(i=global_i,
                        ...
                        runtime_type=runtime_type, amount_of_workers_per_gpu=workers_per_node//amount_of_gpus_per_node))
