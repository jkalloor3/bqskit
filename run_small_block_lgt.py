...
def create_pauli_noise_model(rb_err_1q, rb_err_2q):
...
noise_model = create_pauli_noise_model(1e-4, 1e-3)
...
def get_obs(estimator: Estimator, circs: list[QuantumCircuit], shots: int) -> list[float]:
    '''
    Get the observable for the output density matrix.
    '''
    run_circs = [(transpile(c, basis_gates=["cx", "u"]), ham) for c in circs]
    estimator.options.default_shots = shots
    job = estimator.run(run_circs)
    pub_results = [job.result()[i].data.evs for i in range(len(run_circs))]
    return pub_results
...
if __name__ == '__main__':
    ...
    # target_obs_mag = get_obs(get_density_matrix(target_sv))
    backend = AerSimulator(noise_model=noise_model)
    estimator = Estimator(backend, options={"default_shots": int(shots)})
    target_obs_mag = get_obs(estimator, [full_qc], shots=shots)[0]

    print("Target Observable Magnitude: ", target_obs_mag)
    print("Large Block Names: ", large_block_nums)

    ensemble_sizes = [1, 2, 4, 16, 32, 64, 128, 256, 512, 1024]
    num_trials = 10
    for ens_size in ensemble_sizes:
        ens_data_file = f"ensemble_costs_{circ_name}_obs_noisy/{max_tol}_{ens_size}.pkl"
        ...
        for large_block_num in large_block_nums:
            ...
            for i in range(num_trials):
                ...
                num_shots = shots // len(qcircs)
                ens_mags = get_obs(estimator, qcircs, shots=num_shots)
                # Get the average density matrix
                mag = np.mean(ens_mags)
                ensemble_mags.append(mag)
                print(f"Ens Size: {ens_size} Num Shots: {num_shots} Trial {i+1}/{num_trials} - Mag: {mag:.4f}", flush=True)
